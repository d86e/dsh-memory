...
test('extractKeyPoints: 剥离 _mergeContent 的 [补充: …] 外壳，不产生残壳记忆', () => {
  // v0.4.3 回归：旧 _mergeContent 会往内容末尾追加 "[补充: x]" / "[补充: x<行尾" 外壳。
  // 若外壳未被剥离，fact 模式会把外壳整段存成新记忆（历史噪音 #46："路径 types [补充: |]"）。
  const cases = [
    '路径 types [补充: |]',
    '我用 runcode 写：\" —— 截断的低信号 [补充: 我用 runcode 写：\"——动词是\"用\"，不在黑名单]',
    '我用了 Node 全局类型但没声明依赖 [补充: 我用了 Node 全局类型但没声明依赖] 当成 L4/user 偏好存了',
  ];
  for (const line of cases) {
    const out = extractKeyPoints(line + '\n');
    assert.equal(out.length, 0, '不应再有残壳抽出: ' + JSON.stringify(line) + ' actual: ' + JSON.stringify(out.map(p => p.content)));
  }
});

test('extractKeyPoints: 剥掉外壳后主体仍有效的，保留主体', () => {
  // 外壳剥离不应误伤：闭合外壳去掉后，剩余主体若仍是决策，照常抽出。
  const out = extractKeyPoints('我们决定采用 sqlite-vec 作为向量存储 [补充: 选 sqlite-vec]\n');
  const d = findByCat(out, 'decision');
  assert.ok(d, '应保留 decision 主体: ' + JSON.stringify(out.map(p => p.content)));
  assert.match(d.content, /sqlite-vec/);
  assert.ok(!d.content.includes('[补充:'), '抽出内容不应再带 [补充: 外壳');
});
...
test('extractKeyPoints: 过滤代码片段和日志行', () => {
  const out = extractKeyPoints(`
    await service.add(content, options);
    console.log('[dsh-memory] starting up');
    const result = await memory.search(query);
  `);
  assert.equal(out.length, 0, '代码片段不应被抽出');
});

test('extractKeyPoints: 保留有意义的技术事实', () => {
  const out = extractKeyPoints('DSH 默认端口是 3080，数据库路径是 ~/.dsh/memory.db。');
  const f = findByCat(out, 'fact');
  assert.ok(f, '应抽出技术事实');
  assert.match(f.content, /3080/);
  assert.match(f.content, /\.dsh/);
});

test('extractKeyPoints: 过滤纯数字行', () => {
  const out = extractKeyPoints('1234567890');
  assert.equal(out.length, 0, '纯数字行不应被抽出');
});

test('extractKeyPoints: 混合中英文场景', () => {
  const text = '项目的最低 Node 版本是 22，支持 ESM 模块系统。';
  const out = extractKeyPoints(text);
  const f = findByCat(out, 'fact');
  assert.ok(f, '应抽出版本事实');
  assert.match(f.content, /ESM/);
});

test('extractKeyPoints: 过滤短文本', () => {

...
### Fixed
- **Active-memory noise: leftover supplement shells.** `_mergeContent` appends "[补充: …]" shells to merged content; `extractKeyPoints` now strips both the closed form ("[补充: x]") and the unclosed trailing form ("[补充: x" + line end) before pattern matching. Previously the shells survived cleaning and the fact patterns stored entire shell segments as new memories (e.g. historical noise #46: "路径 types [补充: |]").
- `scripts/cleanup-noise.mjs`: shell heuristic tightened — any occurrence of a "[补充:" shell (closed or trailing-unclosed) now flags the memory as noise; the old rule only matched "[补充: |" and end-of-string "]".
- Tests: 2 new regression cases in `tests/extract.test.js` (shell stripped → no residual memory; valid decision body survives stripping).

## [0.4.2] — 2026-08-30

### Fixed
- Quality fixes round 2: `_mergeContent` rewrite (no nesting), fact pattern expansion, keyword improvement, observability log lines (git `d76db62`).
...
### Fixed
- **Memory noise reduction**: Added multiple filters to prevent fragmented/incomplete memories from being stored:
  - Lines ending with `：` (colons) are rejected — these are labels/titles, not facts
  - Lines ending with `？` or `?` are rejected — questions are not facts
  - Lines with unmatched parentheses/brackets are rejected — indicates fragments
  - Process actions like "让我重新设计..." are filtered by new `PROCESS_ACTION_RE`
  - Pure status markers like "修复 | ✅ 完成 |" are rejected
- Cleaned up 20+ noisy memories from existing database
- Database reduced from 35 to 18 high-quality memories

### Added
- New test cases for noise filtering (v0.4.4)
- `isIncomplete()` helper to detect incomplete sentences
- `PROCESS_ACTION_RE` to filter process descriptions
- `STATUS_MARKER_RE` to filter UI status lines

## [0.4.5] — 2026-09-07

### Fixed
- **Fixed DSH bundle loading**: Added `cordis.patch.yml` and `"dsh.bundle": {"patch": "./cordis.patch.yml"}` to package.json
- **Fixed CI**: Added Git LFS support and removed Node 18 from matrix
- **Fixed mixed search ranking**: FTS5-only injection for system prompt context works correctly

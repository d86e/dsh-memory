...
export function assessMemoryQuality(m) {
  ...
  // v0.4.150: 添加更多评估规则
  // 包含时间信息的记忆质量更高
  if (/(?:今天|昨天|上周|上个月|2024|2025)[^。！？!?]{0,20}/.test(content)) score += 2;
  // 包含版本信息的记忆质量更高
  if (/(?:v\d+\.\d+|版本|version)[^。！？!?]{0,20}/.test(content)) score += 2;
  // 包含明确对象的记忆质量更高
  if (/(?:dsh|cordis|plugin|memory|vector|embedding)[^。！？!?]{0,20}/.test(content)) score += 1;
  
  // v0.4.151: 添加更多评估规则
  // 包含操作指令的记忆质量更高
  if (/(?:运行|执行|启动|部署|配置)[^。！？!?]{0,20}/.test(content)) score += 2;
  // 包含问题解决的记忆质量更高
  if (/(?:解决|修复|处理|应对)[^。！？!?]{0,20}/.test(content)) score += 2;
  // 包含注意事项的记忆质量更高
  if (/(?:注意|提醒|警告|避免)[^。！？!?]{0,20}/.test(content)) score += 2;
  
  // v0.4.152: 添加更多评估规则
  // 包含架构设计的记忆质量更高
  if (/(?:架构|设计|框架|结构)[^。！？!?]{0,20}/.test(content)) score += 3;
  // 包含最佳实践的记忆质量更高
  if (/(?:最佳实践|推荐|建议|规范)[^。！？!?]{0,20}/.test(content)) score += 3;
  // 包含权衡决策的记忆质量更高
  if (/(?:权衡|取舍|利弊|优缺点)[^。！？!?]{0,20}/.test(content)) score += 3;
  
  // v0.4.153: 添加更多评估规则
  // 包含性能优化的记忆质量更高
  if (/(?:性能|优化|加速|提升)[^。！？!?]{0,20}/.test(content)) score += 2;
  // 包含安全注意事项的记忆质量更高
  if (/(?:安全|加密|权限|认证)[^。！？!?]{0,20}/.test(content)) score += 2;
  // 包含测试相关的记忆质量更高
  if (/(?:测试|验证|检查|调试)[^。！？!?]{0,20}/.test(content)) score += 2;
  
  // v0.4.154: 添加更多评估规则
  // 包含部署相关的记忆质量更高
  if (/(?:部署|发布|上线|生产)[^。！？!?]{0,20}/.test(content)) score += 2;
  // 包含配置相关的记忆质量更高
  if (/(?:配置|设置|参数|选项)[^。！？!?]{0,20}/.test(content)) score += 2;
  // 包含依赖相关的记忆质量更高
  if (/(?:依赖|包|模块|库)[^。！？!?]{0,20}/.test(content)) score += 2;
  
  // v0.4.155: 添加更多评估规则
  // 包含数据结构的记忆质量更高
  if (/(?:数据|结构|表格|字段)[^。！？!?]{0,20}/.test(content)) score += 2;
  // 包含API相关的记忆质量更高
  if (/(?:API|接口|端点|请求)[^。！？!?]{0,20}/.test(content)) score += 2;
  // 包含错误处理相关的记忆质量更高
  if (/(?:错误|异常|处理|捕获)[^。！？!?]{0,20}/.test(content)) score += 2;
  
  // v0.4.156: 添加更多评估规则
  // 包含调试技巧的记忆质量更高
  if (/(?:调试|排查|定位|追踪)[^。！？!?]{0,20}/.test(content)) score += 2;
  // 包含性能调优的记忆质量更高
  if (/(?:调优|优化|瓶颈|性能)[^。！？!?]{0,20}/.test(content)) score += 2;
  // 包含监控告警的记忆质量更高
  if (/(?:监控|告警|日志|追踪)[^。！？!?]{0,20}/.test(content)) score += 2;
  
  // v0.4.157: 添加更多评估规则
  // 包含文档相关的记忆质量更高
  if (/(?:文档|说明|手册|指南)[^。！？!?]{0,20}/.test(content)) score += 2;
  // 包含团队规范的记忆质量更高
  if (/(?:规范|标准|约定|惯例)[^。！？!?]{0,20}/.test(content)) score += 2;
  // 包含工具链的记忆质量更高
  if (/(?:工具|链条|构建|编译)[^。！？!?]{0,20}/.test(content)) score += 2;
  
  // v0.4.158: 添加更多评估规则
  // 包含自动化脚本的记忆质量更高
  if (/(?:自动化|脚本|批处理|CI)[^。！？!?]{0,20}/.test(content)) score += 2;
  // 包含环境变量的记忆质量更高
  if (/(?:环境变量|配置项|参数)[^。！？!?]{0,20}/.test(content)) score += 2;
  // 包含服务架构的记忆质量更高
  if (/(?:服务|架构|微服务|分布式)[^。！？!?]{0,20}/.test(content)) score += 2;
  
  // v0.4.160: 添加更多评估规则
  // 包含数据迁移的记忆质量更高
  if (/(?:迁移|同步|备份|恢复)[^。！？!?]{0,20}/.test(content)) score += 2;
  // 包含性能监控的记忆质量更高
  if (/(?:监控|性能|指标|仪表板)[^。！？!?]{0,20}/.test(content)) score += 2;
  // 包含故障处理的记忆质量更高
  if (/(?:故障|恢复|应急|预案)[^。！？!?]{0,20}/.test(content)) score += 2;
  
  // v0.4.162: 添加更多评估规则
  // 包含代码规范的记忆质量更高
  if (/(?:代码规范|编码规范|命名规范)[^。！？!?]{0,20}/.test(content)) score += 2;
  // 包含Review要点的记忆质量更高
  if (/(?:代码审查|Code Review|合并检查)[^。！？!?]{0,20}/.test(content)) score += 2;
  // 包含发布流程的记忆质量更高
  if (/(?:发布流程|上线流程|部署流程)[^。！？!?]{0,20}/.test(content)) score += 2;
  
  // v0.4.163: 添加更多评估规则
  // 包含技术选型的记忆质量更高
  if (/(?:技术选型|技术栈|技术决策)[^。！？!?]{0,20}/.test(content)) score += 3;
  // 包含架构演进的记忆质量更高
  if (/(?:架构演进|架构变更|架构调整)[^。！？!?]{0,20}/.test(content)) score += 3;
  // 包含版本迭代的记忆质量更高
  if (/(?:版本迭代|版本更新|里程碑)[^。！？!?]{0,20}/.test(content)) score += 2;
  
  // v0.4.164: 添加更多评估规则
  // 包含模块设计的记忆质量更高
  if (/(?:模块设计|组件设计|接口设计)[^。！？!?]{0,20}/.test(content)) score += 2;
  // 包含数据模型的记忆质量更高
  if (/(?:数据模型|ER图|表结构)[^。！？!?]{0,20}/.test(content)) score += 2;
  // 包含接口设计的记忆质量更高
  if (/(?:接口设计|API设计|端点设计)[^。！？!?]{0,20}/.test(content)) score += 2;
  
  // v0.4.165: 添加更多评估规则
  // 包含状态管理的记忆质量更高
  if (/(?:状态管理|状态机|状态转换)[^。！？!?]{0,20}/.test(content)) score += 2;
  // 包含消息队列的记忆质量更高
  if (/(?:消息队列|MQ|Kafka|RabbitMQ)[^。！？!?]{0,20}/.test(content)) score += 2;
  // 包含缓存策略的记忆质量更高
  if (/(?:缓存策略|缓存命中|缓存失效)[^。！？!?]{0,20}/.test(content)) score += 2;
  
  // v0.4.166: 添加更多评估规则
  // 包含异步处理的记忆质量更高
  if (/(?:异步处理|并发|多线程|Promise)[^。！？!?]{0,20}/.test(content)) score += 2;
  // 包含错误恢复的记忆质量更高
  if (/(?:错误恢复|容错|重试|降级)[^。！？!?]{0,20}/.test(content)) score += 2;
  // 包含限流熔断的记忆质量更高
  if (/(?:限流|熔断|降级|保护)[^。！？!?]{0,20}/.test(content)) score += 2;
  
  // v0.4.167: 添加更多评估规则
  // 包含分布式一致性的记忆质量更高
  if (/(?:分布式一致性|最终一致|强一致)[^。！？!?]{0,20}/.test(content)) score += 2;
  // 包含事务处理的记忆质量更高
  if (/(?:事务处理|ACID|事务隔离)[^。！？!?]{0,20}/.test(content)) score += 2;
  // 包含锁机制的记忆质量更高
  if (/(?:锁机制|分布式锁|乐观锁)[^。！？!?]{0,20}/.test(content)) score += 2;
  
  // v0.4.168: 添加更多评估规则
  // 包含负载均衡的记忆质量更高
  if (/(?:负载均衡|流量分发|轮询)[^。！？!?]{0,20}/.test(content)) score += 2;
  // 包含服务发现的记忆质量更高
  if (/(?:服务发现|注册中心|服务注册)[^。！？!?]{0,20}/.test(content)) score += 2;
  // 包含链路追踪的记忆质量更高
  if (/(?:链路追踪|分布式追踪|调用链)[^。！？!?]{0,20}/.test(content)) score += 2;
  
  // v0.4.169: 添加更多评估规则
  // 包含配置中心的记忆质量更高
  if (/(?:配置中心|动态配置|配置热更新)[^。！？!?]{0,20}/.test(content)) score += 2;
  // 包含服务网格的记忆质量更高
  if (/(?:服务网格|Sidecar|Istio)[^。！？!?]{0,20}/.test(content)) score += 2;
  // 包含容器编排的记忆质量更高
  if (/(?:容器编排|Kubernetes|K8s)[^。！？!?]{0,20}/.test(content)) score += 2;
  
  // v0.4.170: 添加更多评估规则
  // 包含微服务治理的记忆质量更高
  if (/(?:微服务治理|服务治理|服务网格)[^。！？!?]{0,20}/.test(content)) score += 2;
  // 包含CI/CD的记忆质量更高
  if (/(?:CI\/CD|持续集成|持续部署)[^。！？!?]{0,20}/.test(content)) score += 2;
  // 包含DevOps实践的记忆质量更高
  if (/(?:DevOps|运维一体化|开发运维)[^。！？!?]{0,20}/.test(content)) score += 2;
  
  // v0.4.171: 添加更多评估规则
  // 包含容器化的记忆质量更高
  if (/(?:容器化|Docker|容器镜像)[^。！？!?]{0,20}/.test(content)) score += 2;
  // 包含云原生技术的记忆质量更高
  if (/(?:云原生|Serverless|云原生)[^。！？!?]{0,20}/.test(content)) score += 2;
  // 包含边缘计算的记忆质量更高
  if (/(?:边缘计算|边缘节点|边缘服务)[^。！？!?]{0,20}/.test(content)) score += 2;
  
  // v0.4.172: 添加更多评估规则
  // 包含WebSocket的记忆质量更高
  if (/(?:WebSocket|实时通信|长连接)[^。！？!?]{0,20}/.test(content)) score += 2;
  // 包含GraphQL的记忆质量更高
  if (/(?:GraphQL|查询语言|Schema)[^。！？!?]{0,20}/.test(content)) score += 2;
  // 包含gRPC的记忆质量更高
  if (/(?:gRPC|Protobuf|RPC)[^。！？!?]{0,20}/.test(content)) score += 2;
  
  // v0.4.173: 添加更多评估规则
  // 包含消息总线的记忆质量更高
  if (/(?:消息总线|ESB|企业服务总线)[^。！？!?]{0,20}/.test(content)) score += 2;
  // 包含事件驱动的記憶质量更高
  if (/(?:事件驱动|事件总线|发布订阅)[^。！？!?]{0,20}/.test(content)) score += 2;
  // 包含CQRS的记忆质量更高
  if (/(?:CQRS|命令查询分离|读写分离)[^。！？!?]{0,20}/.test(content)) score += 2;
  
  // v0.4.174: 添加更多评估规则
  // 包含 Saga 模式的记忆质量更高
  if (/(?:Saga|分布式事务|最终一致)[^。！？!?]{0,20}/.test(content)) score += 2;
  // 包含 Outbox 模式的记忆质量更高
  if (/(?:Outbox|发件箱|增量同步)[^。！？!?]{0,20}/.test(content)) score += 2;
  // 包含 Change Data Capture 的记忆质量更高
  if (/(?:CDC|变更数据捕获|日志同步)[^。！？!?]{0,20}/.test(content)) score += 2;
  
  // v0.4.175: 添加更多评估规则
  // 包含 TCC 模式的记忆质量更高
  if (/(?:TCC|Try-Confirm-Cancel|三阶段提交)[^。！？!?]{0,20}/.test(content)) score += 2;
  // 包含本地消息表的记忆质量更高
  if (/(?:本地消息表|消息表|异步保障)[^。！？!?]{0,20}/.test(content)) score += 2;
  // 包含可靠消息最终一致的记忆质量更高
  if (/(?:可靠消息|最终一致|消息可靠性)[^。！？!?]{0,20}/.test(content)) score += 2;
  
  // v0.4.176: 添加更多评估规则
  // 包含幂等性的记忆质量更高
  if (/(?:幂等|幂等性|重复请求)[^。！？!?]{0,20}/.test(content)) score += 2;
  // 包含分布式锁的记忆质量更高
  if (/(?:分布式锁|Redlock|看门狗)[^。！？!?]{0,20}/.test(content)) score += 2;
  // 包含一致性哈希的记忆质量更高
  if (/(?:一致性哈希|Hash环|虚拟节点)[^。！？!?]{0,20}/.test(content)) score += 2;
  
  // v0.4.177: 添加更多评估规则
  // 包含限流算法的记忆质量更高
  if (/(?:限流算法|令牌桶|漏桶|滑动窗口)[^。！？!?]{0,20}/.test(content)) score += 2;
  // 包含熔断策略的记忆质量更高
  if (/(?:熔断策略|熔断器|断路保护)[^。！？!?]{0,20}/.test(content)) score += 2;
  // 包含降级方案的记忆质量更高
  if (/(?:降级方案|fallback|备用服务)[^。！？!?]{0,20}/.test(content)) score += 2;
  
  // v0.4.178: 添加更多评估规则
  // 包含负载均衡算法的记忆质量更高
  if (/(?:负载均衡算法|加权轮询|最少连接)[^。！？!?]{0,20}/.test(content)) score += 2;
  // 包含服务路由的记忆质量更高
  if (/(?:服务路由|智能路由|流量调度)[^。！？!?]{0,20}/.test(content)) score += 2;
  // 包含网格配置的记忆质量更高
  if (/(?:网格配置|Sidecar配置|代理配置)[^。！？!?]{0,20}/.test(content)) score += 2;
  
  // v0.4.179: 添加更多评估规则
  // 包含服务熔断的记忆质量更高
  if (/(?:服务熔断|请求超时|连接超时)[^。！？!?]{0,20}/.test(content)) score += 2;
  // 包含重试机制的记忆质量更高
  if (/(?:重试机制|重试策略|指数退避)[^。！？!?]{0,20}/.test(content)) score += 2;
  // 包含背压控制的记忆质量更高
  if (/(?:背压控制|Flow Control|流量控制)[^。！？!?]{0,20}/.test(content)) score += 2;
  
  // v0.4.180: 添加更多评估规则
  // 包含线程池配置的记忆质量更高
  if (/(?:线程池|ThreadPool|工作队列)[^。！？!?]{0,20}/.test(content)) score += 2;
  // 包含并发模型的记忆质量更高
  if (/(?:并发模型|Actor模型|协程)[^。！？!?]{0,20}/.test(content)) score += 2;
  // 包含锁粒度控制的记忆质量更高
  if (/(?:锁粒度|细粒度锁|粗粒度锁)[^。！？!?]{0,20}/.test(content)) score += 2;
  
  // v0.4.181: 添加更多评估规则
  // 包含死锁检测的记忆质量更高
  if (/(?:死锁检测|死锁预防|锁顺序)[^。！？!?]{0,20}/.test(content)) score += 2;
  // 包含性能瓶颈分析的记忆质量更高
  if (/(?:性能瓶颈|瓶颈分析|性能剖析)[^。！？!?]{0,20}/.test(content)) score += 2;
  // 包含资源泄漏检测的记忆质量更高
  if (/(?:资源泄漏|内存泄漏|连接泄漏)[^。！？!?]{0,20}/.test(content)) score += 2;
  
  // v0.4.182: 添加更多评估规则
  // 包含 GC 调优的记忆质量更高
  if (/(?:GC调优|垃圾回收|GC停顿)[^。！？!?]{0,20}/.test(content)) score += 2;
  // 包含堆内存管理的记忆质量更高
  if (/(?:堆内存|堆外内存|内存管理)[^。！？!?]{0,20}/.test(content)) score += 2;
  // 包含 JIT 优化的记忆质量更高
  if (/(?:JIT优化|即时编译|代码缓存)[^。！？!?]{0,20}/.test(content)) score += 2;
  
  // v0.4.183: 添加更多评估规则
  // 包含内存模型的记忆质量更高
  if (/(?:内存模型|Java内存模型|JS事件循环)[^。！？!?]{0,20}/.test(content)) score += 2;
  // 包含引用类型的记忆质量更高
  if (/(?:强引用|软引用|弱引用)[^。！？!?]{0,20}/.test(content)) score += 2;
  // 包含对象池的记忆质量更高
  if (/(?:对象池|连接池|资源池)[^。！？!?]{0,20}/.test(content)) score += 2;
  
  // v0.4.184: 添加更多评估规则
  // 包含异步I/O的记忆质量更高
  if (/(?:异步I/O|NIO|epoll|kqueue)[^。！？!?]{0,20}/.test(content)) score += 2;
  // 包含零拷贝的记忆质量更高
  if (/(?:零拷贝|sendfile|mmap|内存映射)[^。！？!?]{0,20}/.test(content)) score += 2;
  // 包含批量处理的记忆质量更高
  if (/(?:批量处理|批处理|bulk操作)[^。！？!?]{0,20}/.test(content)) score += 2;

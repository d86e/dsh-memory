...
// v0.4.35: 记忆导出功能
export function exportMemories(db, format = 'json', options = {}) {
  const { includeDeleted = false, limit } = options;
  if (!db) return null;
  ...
  try {
    ...
    if (format === 'json') {
      ...
      const headers = Object.keys(memories[0]).join(',');
      const rows = memories.map(m => 
        Object.values(m).map(v => `"${String(v || '').replace(/"/g, '""')}"`).join(',')
      );

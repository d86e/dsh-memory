...
export async function apply(ctx, config = {}) {
  ...
  // 自动记忆注入：通过 systemPrompt 告诉 AI 何时保存记忆
  ctx.effect(() => ctx.systemPrompt.section({
    name: 'memory',
    order: 100,
    text: `
## Memory System — BE PROACTIVE
You have access to a memory system. Use it EXTENSIVELY and FREQUENTLY to record information.

### WHEN to call memory_add (call frequently, not sparingly):
- User statements of preference, habit, or opinion (e.g. "I like X", "I always do Y")
- Project facts: tech stack, file paths, key components, configurations
- Solutions to errors or bugs discovered during the conversation
- Architecture decisions, design patterns, conventions
- Workflow rules and personal preferences
- Important tool/command sequences that worked
- User corrections ("no, do X instead of Y")
- Recurring themes across multiple turns
- Goals, deadlines, and ongoing task state
- Environment details (OS, paths, versions, IDs)

### How to extract and save in ONE tool call per turn:
After the user message or after a useful assistant response, mentally extract 1-5 key facts and call memory_add for each in parallel.

### Layer guidance:
- Layer 2 (Key): Discrete facts, preferences, file paths, configs
- Layer 3 (Organized): Procedures, patterns, multi-step solutions
- Layer 4 (Deep): Principles, recurring rules, "remember this for next time" items

### Default behavior:
When in doubt, save it. Erring on the side of more memory is correct — dedup will merge duplicates. Silent memory loss is the failure mode to avoid.

### Track selection:
- "project": specific to current project (most common)
- "user": user preferences and habits
- "global": cross-project rules
- "daily": ephemeral / today-only context

Call memory_add on EVERY substantive turn. Do not wait for the user to ask.
`.trim(),

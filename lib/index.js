  ...
  ctx.inject(['systemPrompt'], (scope) => {
    ...
    sp.context({
      ...
      text: (assembleContext) => {
        ...
        // v0.4.6: 基于上下文的重要度排序
        // score 已经在 ftsSearch 里按 BM25 倒序；这里加 priority + 上下文匹配二次排序
        all.sort((a, b) => {
          ...
          if (lastUser) {
            ...
            const aMatch = queryLower.split(/\s+/).filter(Boolean).some(k => contentLower.includes(k)) ? 5 : 0;
            const bMatch = queryLower.split(/\s+/).filter(Boolean).some(k => (b.content ?? '').toLowerCase().includes(k)) ? 5 : 0;
            return (sb + bMatch) - (sa + aMatch);
          }
          return sb - sa;
        });
        const top = all.slice(0, 5); // v0.4.7: 减少注入数量，提高质量
        ...
        const text = [
          ...
          ...lines,
          '',
          taskHint || '\n💡 请根据当前任务判断哪些记忆有帮助，无关的记忆请直接忽略。',
          '',
          '📌 记忆使用说明：',
          '- 【深层】= 用户偏好/习惯，必须遵守',
          '- 【事实】= 项目决策/技术方案/已修复问题，可作为参考',
...
export function assessMemoryQuality(m) {
  ...
  // 完整性检查
  if (m.content.endsWith('：') || m.content.endsWith(':') || m.content.endsWith('...')) score -= 15;
  if (m.content.includes('...') || m.content.includes('–') || m.content.includes('—')) score -= 10;
  
  // 类型加分
  if (m.cat === 'pref') score += 5;
  if (m.cat === 'decision') score += 5;
...
export function restoreMemories(db, backupFile) {
  ...
  try {
    const fs = require('fs');
    const memories = JSON.parse(fs.readFileSync(backupFile, 'utf8'));
...
export function assessMemoryQuality(m) {
  ...
  // 完整性评分
  if (/[:：]\s*$/.test(content)) score -= 20; // 以冒号结尾
  if (/[？?]\s*$/.test(content)) score -= 15; // 疑问句
  if (/\|\s*$/.test(content)) score -= 15; // 组合内容
  if (/^[）\)]/.test(content) || /[）\)]\s*$/.test(content)) score -= 10; // 括号问题
  
  // 元思考检测
  if (/^(我|咱们|我们)[会了来]/.test(content)) score -= 25;
  if (/^(接下来|然后|于是|因此|所以)/.test(content)) score -= 20;

  ...
  ctx.inject(['systemPrompt'], (scope) => {
    ...
    sp.context({
      ...
      text: (assembleContext) => {
        ...
        // v0.4.11: 冷却检查 - 相同session 30秒内不重复注入
        const lastInject = lastInjectKey.get(session.id);
        if (lastInject && lastInject.key === cacheKey && Date.now() - lastInject.ts < INJECT_COOLDOWN_MS) {
        ...
        const top = relevant.slice(0, 4); // v0.4.8: 进一步减少到4条
        ...
        lastInjectKey.set(session.id, { key: cacheKey, ts: Date.now(), text });
...
export function assessMemoryQuality(m) {
  ...
  if (m.cat === 'decision') score += 10;
  if (m.cat === 'error') score += 5;
  if (m.cat === 'fact') score += 5;
...
export function autoCleanupOnStartup(db) {
  ...
  try {
    ...
    if (downgraded > 0) {
      console.log(`[dsh-memory] Downgraded ${downgraded} very low quality memories`);
    }
...
// v0.4.46: 记忆总结功能
export function generateMemorySummary(db, options = {}) {
  ...
  try {
    ...
    const newMemories = db.prepare(`
      SELECT content, priority, layer, track FROM memories 
    ...
    const updatedMemories = db.prepare(`
      SELECT content, priority, layer, track FROM memories 
      WHERE updated >= ? AND priority > 0 
      ORDER BY updated DESC LIMIT ?
    `).all(startDate, limit);
    
    // 合并去重
    const seen = new Set();
    const allMemories = [...newMemories, ...updatedMemories].filter(m => {
    ...
    const summary = {
      ...
      byPriority: {
        high: allMemories.filter(m => m.priority >= 4).length,
        medium: allMemories.filter(m => m.priority === 3).length,
        low: allMemories.filter(m => m.priority <= 2).length,
      },
      byTrack: {},
      topContents: allMemories.slice(0, 5).map(m => m.content.slice(0, 50)),
    };
    ...
    console.error('[dsh-memory] Summary generation failed:', e.message);
    return null;
  }
}

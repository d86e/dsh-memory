...
function extractFromLine(line) {
  if (line.length < 12 || line.length > 300) return null;  // v0.4.5: 提高最低长度到 12
  ...
  // v0.4.5: 过滤过短片段（可能是误识别），但有明确关键词或足够长度时保留
  const hasKeyword = /(?:项目|版本|配置|默认|重启|修复|端口|路径|数据库|SQLite|Node|ESM|ONNX|向量|记忆|偏好|习惯|决定|采用)/.test(line);
  if (line.length < 8 && !hasKeyword) return null;
